...
char simbolos[11][11][56]; //Un grafo en matriz, esto para hacer mas facil guardar la informacion
...
char dfa[11][11][56];
int localizaciones[10];
int loc = 0;
char test[] = "(i·f)|(i·n·t)"; // Input donde etson probara todo
...
for(int xu=0;xu<11;xu++){
  for(int qe=0;qe<11;qe++){
    strcpy(simbolos[xu][qe], "");
    strcpy(dfa[xu][qe], "");
  }
}
...
if (guardando){
  loc = 0;
  for(int d=0;d<10;d++){if(localizaciones[d] != 0){localizaciones[d] = 0;}else{break;}}
guardar_x = ini; // Este es para guardar nodo inicial en caso vinieran OR
localizaciones[loc] = dest;
loc +=1 ;
}
else if (ramificar){
...
if(dest-ini >= 2){
  ...
  for(int b=0;b<loc;b++){
    if(noiterar){break;}
    for(int r=1+b;r<loc;r++){
    if(simbolos[ini][localizaciones[b]][0] == simbolos[ini][localizaciones[r]][0]){
      if(test[rec] > 47 && test[rec] < 58){
        if(test[rec+1] == '-'){str += test[rec]; str += test[rec+1]; str += test[rec+2]; }
      }
      else if(test[rec+1] == 36 || test[rec+1] == -62){printf("ASIGNANDO");str += test[rec+2];}
      strcat(simbolos[localizaciones[b]][localizaciones[r]+1], str.c_str());
      str = "";
      noiterar = true;
      break;
    }
...
int h=0;
std::string str("");
while(continuar && t<11){
state_actual = 0;
if(simbolos[r][t][0]){
  if(simbolos[t][r][0] == '#'){printf("\nENTRE A SIEMPRE EPSILON t: %d, r: %d",t,r);siempre = true;}
  h=0;
  while(estar){
    if(dejar){pasar += 1;printf("P%d M%d",pasar, medir);if(pasar <= medir){break;}else{dejar=false;ini_2=r;dest_2=t;}}
    if(simbolos[r][t][h]){
      if(simbolos[r][t][h] == '|'){
        if(simbolos[r][t][h+1] == '#'){
          printf("\nENTRE A | EN #");
          ...
          printf("\nENTRE A | EN VARIABLES");
          str += simbolos[r][t][h+1];
          strcat(dfa[ini_2][dest_2], str.c_str());
          str = "";
        }
        h += 1;
      }
      else if(simbolos[r][t][h] == '#'){
        printf("\nENTRE A EPSILON");
        state_actual = t;
        ini_2 -= 1;
        dest_2 -= 1;
        ...
        printf("\nENTRE A GUARDAR DE UNA");
        str += simbolos[r][t][h];
        if (guardardoble){printf("\nENTRE A GUARDADOBLE");
          strcat(dfa[ini_2-1][dest_2], str.c_str());
          if(!simbolos[r][t][h+1]){printf("\nENTRE A QUITAR GUARDARDOBLE");guardardoble = false;}
        }
        strcat(dfa[ini_2][dest_2], str.c_str());
        if(siempre){printf("\nENTRE A SIEMPRE");strcat(dfa[dest_2][dest_2], str.c_str());if(!simbolos[r][t][h+1]){printf("\nENTRE A QUITAR SIEMPRE");siempre = false;}}
        str = "";
        if(dest_2-ini_2 >= 2){ini_2 = dest_2-1;}
        state = true;
      }
    }
    else{
...
}if(!state_actual){t += 1;if(state){r=t-1;ini_2 += 1;dest_2 += 1;state = false;}}else{r = state_actual;ini_2 += 1;dest_2 += 1;state=false;}}
if(eps){
  printf("EPS: %d",eps);
  dejar = true;
  medir += 1;
